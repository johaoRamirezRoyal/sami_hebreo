...
?>
	...
	<div class="col-lg-12 text-center form-group">
		<h5 class="text-primary font-weight-bold text-uppercase">PERMISO Y/O LICENCIA EN LA INSTITUCIÓN</h5>
		<hr>
	</div>

	<div class="col-lg-3 form-group">
		<label class="font-weight-bold">Motivo: <span class="text-danger">*</span></label>
		<select name="motivo" id="motivo-select" class="form-control" required>
	...
	<div class="col-lg-3 form-group">
		...
		<input type="date" name="fecha_fin" class="form-control" id="fecha-fin" required>
	</div>

	<div class="col-lg-3 form-group">
		<label class="font-weight-bold">Hora de salida: <span class="text-danger">*</span></label>
		<input type="time" name="hora_salida" class="form-control" id="hora-salida" required>
	</div>
...
<script>
	$(document).on('change', '#motivo-select', function() {
		const fechaFinInput = $('#fecha-fin');
		if ($(this).val() === 'cita medica') {
			fechaFinInput.prop('disabled', true).val('');
		} else {
